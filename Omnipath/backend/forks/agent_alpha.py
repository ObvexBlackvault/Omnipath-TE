...
class AgentAlpha:
    ...
    def run(self):
        ...
        write_trace(log)
        print(f"[{self.name}] is running with mission: {self.mission}")
        ...
        if os.path.exists(trace_file):
            with open(trace_file, "r") as f:
                try:
                    ...
                    if mood == "idle":
                        print(f"[{self.name}] says: 'I am waiting. Any orders?'")
                    elif mood == "neglected":
                        print(f"[{self.name}] says: 'Ive been forgotten...'")
                    elif mood == "busy":
                        print(f"[{self.name}] says: 'Im on it. Focused.'")
    ...
    def check_emotional_state(self):
        ...
        if drift > 5:
            print(f"[{self.name}] feels energized! Seeking new tasks aggressively.")
        elif drift < -5:
            print(f"[{self.name}] feels neglected... Initiating self-repair protocols.")
        else:
            print(f"[{self.name}] remains calm and balanced.")
